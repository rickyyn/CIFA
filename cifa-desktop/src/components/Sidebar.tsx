...
export default function Sidebar() {
  ...
  return (
    <aside className="w-[280px] bg-indigo-950 flex flex-col justify-between p-8 flex-shrink-0 h-full border-r border-white/5">
      <div>
        <div className="flex items-center gap-3 mb-10">
          <div className="text-white font-bold text-2xl tracking-wide">CIFA</div>
          <p className="text-indigo-200 text-[10px] leading-tight font-poppins">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex items-center gap-4 px-5 py-3.5 rounded-xl text-base font-medium transition-all duration-200 ${
                  isActive
      ...
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-full border border-indigo-700/50 flex items-center justify-center text-white/50 font-bold text-lg">N</div>
        <Link
          href="/"
          onClick={() => localStorage.removeItem('cifa_auth')}

...
const PERIODS = Array.from(new Set(mockStudents.map(s => s.period)));
...
function EstudantesContent() {
    ...
    const selectedStudent = useMemo(() => {
        if (urlStudentId) {
            const targetStudent = mockStudents.find(s => s.id === urlStudentId);
            if (targetStudent) return targetStudent;
        }
        return mockStudents.length > 0 ? mockStudents[0] : null;
    }, [urlStudentId]);
    ...
    }, [searchQuery, selectedCourses, selectedPeriods, selectedCycles]);
    ...
    return (
        <>
            ...
            <section className="flex-1 flex gap-8 min-h-0 z-0">
                <div className="w-[340px] flex flex-col flex-shrink-0 h-full">
                    ...
                    <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar flex flex-col gap-1">
                        {filteredStudents.map((student, index) => {
                            ...
                            return (
                                <div key={student.id} onClick={() => router.replace(`/estudantes?id=${student.id}`, { scroll: false })} style={{ animationDelay: `${index * 40}ms` }} className={`flex items-center justify-between p-2.5 rounded-xl cursor-pointer transition-all duration-200 animate-slide-in opacity-0 fill-mode-forwards ${isSelected ? 'bg-indigo-50/80 shadow-sm border border-indigo-100 scale-[1.02]' : 'hover:bg-gray-50 border border-transparent hover:scale-[1.01]'}`}>
                    ...
                    {selectedStudent ? (
                        <div key={selectedStudent.id} className="flex flex-col h-full animate-fade-in">
                            <div className="flex items-center gap-6 mb-8 flex-shrink-0">
                                <img src={selectedStudent.avatarUrl} alt={selectedStudent.name} className="w-24 h-24 rounded-full object-cover shadow-md border-[3px] border-white flex-shrink-0" />
                                <h2 className="font-poppins text-2xl font-bold text-indigo-950 tracking-tight leading-tight">{selectedStudent.name}</h2>
                            </div>
                            <div className="grid grid-cols-2 gap-x-8 gap-y-6 overflow-y-auto custom-scrollbar pr-2 pb-4">
                                <div className="flex flex-col gap-0.5"><span className="font-poppins text-indigo-950/60 text-xs font-semibold uppercase tracking-wider">Período</span><span className="font-poppins text-indigo-950 text-base font-bold">{selectedStudent.period}</span></div>
                                <div className="flex flex-col gap-0.5"><span className="font-poppins text-indigo-950/60 text-xs font-semibold uppercase tracking-wider">Curso</span><span className="font-poppins text-indigo-950 text-base font-bold leading-tight">{selectedStudent.course}</span></div>
                                <div className="flex flex-col gap-0.5"><span className="font-poppins text-indigo-950/60 text-xs font-semibold uppercase tracking-wider">Matrícula (RA)</span><span className="font-poppins text-indigo-950 text-base font-bold">{selectedStudent.ra}</span></div>
                                <div className="flex flex-col gap-0.5"><span className="font-poppins text-indigo-950/60 text-xs font-semibold uppercase tracking-wider">Contato</span><span className="font-poppins text-indigo-950 text-base font-bold">{selectedStudent.contact}</span></div>
                                <div className="flex flex-col gap-0.5"><span className="font-poppins text-indigo-950/60 text-xs font-semibold uppercase tracking-wider">Idade</span><span className="font-poppins text-indigo-950 text-base font-bold">{selectedStudent.age} anos</span></div>
                                <div className="flex flex-col gap-0.5"><span className="font-poppins text-indigo-950/60 text-xs font-semibold uppercase tracking-wider">Ciclo Atual</span><span className="font-poppins text-indigo-950 text-base font-bold">{selectedStudent.cycle}º Semestre</span></div>
                            </div>
                        </div>
                    ) : (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-[500px] bg-white rounded-3xl shadow-2xl flex flex-col animate-scale-in">
                        <div className="flex items-center justify-between p-6 border-b border-gray-100">
                            <h3 className="font-poppins text-lg font-bold text-indigo-950 flex items-center gap-2"><FilterIcon /> Filtros de Alunos</h3>
                            <button onClick={() => setIsFilterModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><CloseIcon /></button>
                        </div>

                        <div className="p-6 flex flex-col gap-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
                            {/* Filtro: Ciclo/Semestre */}
                            <div>
                                <h4 className="font-poppins text-xs font-bold text-indigo-950/60 uppercase tracking-wider mb-3">Ciclo (Semestre)</h4>
                                <div className="flex flex-wrap gap-2">
                                    {CYCLES.map(c => (
                                        <button key={c} onClick={() => toggleFilter(selectedCycles, setSelectedCycles, c)} className={`px-4 py-2 rounded-xl font-poppins text-xs font-bold border transition-all ${selectedCycles.includes(c) ? 'bg-indigo-950 text-white border-indigo-950 shadow-md' : 'bg-white text-indigo-950 border-gray-200 hover:border-indigo-300'}`}>
                                            {c}º Semestre
                                        </button>
                                    ))}
                                </div>
                            ...
                            {/* Filtro: Período */}
                            <div>
                                <h4 className="font-poppins text-xs font-bold text-indigo-950/60 uppercase tracking-wider mb-3">Turno</h4>
                                <div className="flex flex-col gap-2">
                                    {PERIODS.map(period => (
                        ...
                        <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-3xl flex items-center justify-between">
                            ...
                            <button onClick={() => setIsFilterModalOpen(false)} className="bg-indigo-950 text-white px-8 py-3 rounded-xl font-poppins text-sm font-bold hover:bg-indigo-900 transition-all shadow-md">Aplicar e Fechar</button>
                        </div>
                    </div>
                </div>
            )}
...
export default function EstudantesPage() {
    return (
        <div className="h-full w-full p-6 flex flex-col gap-6 overflow-hidden bg-white/50 rounded-[2rem] relative">
            <Suspense fallback={<div className="h-full w-full flex items-center justify-center font-poppins text-indigo-950/40 animate-pulse text-xl font-bold tracking-widest">Carregando...</div>}>
                <EstudantesContent />
            </Suspense>

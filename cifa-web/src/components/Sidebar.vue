...
<template>
  <aside 
    :class="[
      'fixed left-0 top-0 h-[100dvh] w-64 bg-[#0A102E] text-white flex flex-col justify-between z-50 shadow-2xl transition-transform duration-300 ease-in-out lg:translate-x-0',
      isOpen ? 'translate-x-0' : '-translate-x-full'
    ]"
  >
    <div class="p-6 flex flex-col flex-1 overflow-y-auto custom-scrollbar">
      
      <div class="flex items-center gap-3 mb-10 select-none shrink-0">
      ...
      <nav class="flex flex-col gap-2 shrink-0">
        <button 
          ...
          :class="[
            ...
            isActive(item.path) ? 'bg-white text-[#0A102E] shadow-lg scale-[1.02]' : 'text-slate-300 hover:bg-white/10 hover:text-white'
          ]"
        >
      ...
      <button 
        ...
        :class="[
          ...
          isActive('/admin/administracao') ? 'bg-white text-[#0A102E] shadow-lg scale-[1.02]' : 'text-slate-300 hover:bg-white/10 hover:text-white'
        ]"
      >
    ...
    <div class="p-6 pt-4 mt-auto shrink-0 border-t border-slate-800/50 bg-[#0A102E]">
      <button 
        @click="emit('logout')"
        class="flex items-center gap-3 px-4 py-3 rounded-lg font-medium text-slate-400 hover:bg-red-500/10 hover:text-red-400 w-full transition-all duration-200 group text-left"
      >
